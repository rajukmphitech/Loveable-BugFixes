...
const generatePersonalizedContent = async (name: string, industry: string): Promise<string> => {
  // Always return fallback content to ensure we never have undefined
  const fallbackContent = `Hi ${name}! 🚀 Welcome to our innovation community! We're thrilled to have someone from the ${industry} industry join us. Get ready to discover cutting-edge insights, connect with fellow innovators, and unlock new opportunities that will transform how you work. This is just the beginning of your innovation journey!`;
  ...
  try {
    ...
    if (typeof content === 'string' && content.trim()) {
      ...
      console.log('Invalid or empty AI response, using fallback');
      return fallbackContent;
    }
  } catch (error) {
